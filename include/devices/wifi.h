...
typedef struct {
    ...
    char ssid[32 + 1];          /* SSID of the wlan needs to be connected. */
    char password[64 + 1];           /* Security key of the wlan needs to be connected, ignored in an open system. */
 ...
 */
typedef struct {
    void (*sta_connect_fail)(dev_t *dev, wifi_err_code_t err, void *arg);
    void (*status_change)(dev_t *dev, wifi_event_id_t stat, void *arg);
    void (*scan_compeleted)(dev_t *dev, uint16_t number, wifi_ap_record_t *ap_records);
    void (*fatal_err)(dev_t *dev, void *arg);
} wifi_event_func;
...
/** compatibility */
int hal_wifi_set_smartcfg(dev_t *dev, int enable);
