...
#define TRACE_CPU_CLOCK_HZ           drv_get_sys_freq()                             /* Peripheral clock speed in Hertz. */
#define TRACE_PERIPHERAL_CLOCK_HZ    drv_get_sys_freq()                             /* Peripheral clock speed in Hertz. */
...
#define TRACE_CLASS_TIMER ((traceObjectClass)6)
#define TRACE_CLASS_EVENT ((traceObjectClass)7)

#define TRACE_KERNEL_OBJECT_COUNT (NQueue + NSemaphore + NMutex + NTask + NISR + NMem + NTimer + NEvent)
...
#define PropertyTableSizeEvent		(OBJECT_NAME_LENGTH + 1)

/* TaskSem properties (except name):	state (signaled = 1, cleared = 0) */
#define PropertyTableSizeTimer		(OBJECT_NAME_LENGTH + 1)
...
#define StartIndexTimer			StartIndexMem       + NMem			* PropertyTableSizeMem
#define StartIndexEvent			StartIndexTimer     + NTimer		* PropertyTableSizeTimer

/* Number of bytes used by the object table */
#define TRACE_OBJECT_TABLE_SIZE	StartIndexEvent + NEvent * PropertyTableSizeEvent
...
#define TRACE_GET_TASK_NAME(pxTCB) ((char*)pxTCB->task_name)
...
#define TRACE_GET_TIMER_NAME(pxTimer) pxTimer->name
...
#define TRACE_GET_EVENT_GROUP_NUMBER(pevent)    (((kevent_t *)pevent)->eventid)
#define TRACE_SET_EVENT_GROUP_NUMBER(pevent)    (((kevent_t *)pevent)->eventid = trace_get_object_handle(TRACE_CLASS_EVENT))

#define TRACE_GET_TIMER_NUMBER(ptimer)    (((ktimer_t *)ptimer)->timerid)
#define TRACE_SET_TIMER_NUMBER(ptimer)    (((ktimer_t *)ptimer)->timerid = trace_get_object_handle(TRACE_CLASS_TIMER))

#define TRACE_GET_MUTEX_NUMBER(pmutex)  (((kmutex_t *)pmutex)->mutexid)
#define TRACE_SET_MUTEX_NUMBER(pmutex)  (((kmutex_t *)pmutex)->mutexid = trace_get_object_handle(TRACE_CLASS_MUTEX))

#define TRACE_GET_SEM_NUMBER(psem) (((ksem_t *)psem)->semid)
#define TRACE_SET_SEM_NUMBER(psem) (((ksem_t *)psem)->semid= trace_get_object_handle(TRACE_CLASS_SEMAPHORE))
...
#define TRACE_GET_TIMER_TRACE_CLASS()      TRACE_CLASS_TIMER
#define TRACE_GET_EVENT_GROUP_TRACE_CLASS() TRACE_CLASS_EVENT
...
#define traceEVENT_GROUP_DELETE(CLASS, p_sem) \
   trcKERNEL_HOOKS_OBJECT_DELETE(DELETE_OBJ, CLASS, p_sem);

#undef traceTIMER_DELETE
#define traceTIMER_DELETE(CLASS, p_sem) \
   trcKERNEL_HOOKS_OBJECT_DELETE(DELETE_OBJ, CLASS, p_sem);
...
#define traceTIMER_CREATE(CLASS, p_tcb, p_name )\
...
#define traceEVENT_GROUP_CREATE(CLASS, p_tcb, p_name )\
   TRACE_ENTER_CRITICAL_SECTION(); \
   trcKERNEL_HOOKS_OBJECT_CREATE(CREATE_OBJ, CLASS, p_tcb); \
   trace_set_object_name(TRACE_GET_OBJECT_TRACE_CLASS(CLASS, p_tcb), TRACE_GET_OBJECT_NUMBER(CLASS, p_tcb), p_name); \
   TRACE_EXIT_CRITICAL_SECTION();
...
#define traceQUEUE_SEND( pxQueue ) \
  ...
  trcKERNEL_HOOKS_SET_OBJECT_STATE(CLASS, p_q, (uint8_t)(p_q->msg_q.cur_num));
...
#define traceTASK_MSG_Q_SEND(CLASS, p_msg_q) \
  ...
  trcKERNEL_HOOKS_SET_OBJECT_STATE(CLASS, p_msg_q, (uint32_t)((msg_q_t *)(p_msg_q)->cur_num));
...
#undef traceTIMER_SEND
#define traceTIMER_SEND(CLASS, p_tcb) \
   trcKERNEL_HOOKS_KERNEL_SERVICE(SEND, SUCCESS, CLASS, p_tcb); \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(CLASS, p_tcb, (uint8_t)(p_tcb->SemCtr));

#undef traceTIMER_SEND_FAILED
#define traceTIMER_SEND_FAILED(CLASS, p_tcb) \
...
#define traceSEM_SEND(CLASS, p_sem) \
  ...
  trcKERNEL_HOOKS_SET_OBJECT_STATE(CLASS, p_sem, (uint8_t)(p_sem->count));
...
#undef traceEVENT_GROUP_RECEIVE
#define traceEVENT_GROUP_RECEIVE(CLASS, p_msg_q) \
   trcKERNEL_HOOKS_KERNEL_SERVICE(RECEIVE, SUCCESS, CLASS, p_msg_q); \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(CLASS, p_msg_q, (uint8_t)(p_msg_q->flags));

#undef traceEVENT_GROUP_RECEIVE_BLOCK
#define traceEVENT_GROUP_RECEIVE_BLOCK(CLASS, p_msg_q) \
   trcKERNEL_HOOKS_KERNEL_SERVICE(RECEIVE, BLOCK, CLASS, p_msg_q); \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(CLASS, p_msg_q, (uint8_t)(p_msg_q->flags));
...
#undef traceTIMER_RECEIVE
#define traceTIMER_RECEIVE(CLASS, p_tcb) \
   trcKERNEL_HOOKS_KERNEL_SERVICE(RECEIVE, SUCCESS, CLASS, p_tcb); \
  trcKERNEL_HOOKS_SET_OBJECT_STATE(CLASS, p_tcb, (uint8_t)(p_tcb->remain));

#undef traceTIMER_RECEIVE_FAILED
#define traceTIMER_RECEIVE_FAILED(CLASS, p_tcb) \
   trcKERNEL_HOOKS_KERNEL_SERVICE(RECEIVE, FAILED, CLASS, p_tcb);

/* Called when a task pends and blocks on a task semaphore (Pend)*/
#undef traceTIMER_RECEIVE_BLOCK
#define traceTIMER_RECEIVE_BLOCK(CLASS, p_tcb) \
...
#define TRACE_SET_TIMER_FLAG_ISEXCLUDED(taskSemIndex)   TRACE_SET_FLAG_ISEXCLUDED(excluded_objects,   NQueue+1+NSemaphore+1+NMutex+1+NMem+1+taskSemIndex)
#define TRACE_CLEAR_TIMER_FLAG_ISEXCLUDED(taskSemIndex) TRACE_CLEAR_FLAG_ISEXCLUDED(excluded_objects, NQueue+1+NSemaphore+1+NMutex+1+NMem+1+taskSemIndex)
#define TRACE_GET_TIMER_FLAG_ISEXCLUDED(taskSemIndex)   TRACE_GET_FLAG_ISEXCLUDED(excluded_objects,   NQueue+1+NSemaphore+1+NMutex+1+NMem+1+taskSemIndex)

#define TRACE_SET_EVENT_FLAG_ISEXCLUDED(taskQIndex)   TRACE_SET_FLAG_ISEXCLUDED(excluded_objects,   NQueue+1+NSemaphore+1+NMutex+1+NMem+1+NTimer+1+taskQIndex)
#define TRACE_CLEAR_EVENT_FLAG_ISEXCLUDED(taskQIndex) TRACE_CLEAR_FLAG_ISEXCLUDED(excluded_objects, NQueue+1+NSemaphore+1+NMutex+1+NMem+1+NTimer+1+taskQIndex)
#define TRACE_GET_EVENT_FLAG_ISEXCLUDED(taskQIndex)   TRACE_GET_FLAG_ISEXCLUDED(excluded_objects,   NQueue+1+NSemaphore+1+NMutex+1+NMem+1+NTimer+1+taskQIndex)

#define TRACE_SET_TASK_FLAG_ISEXCLUDED(taskIndex)   TRACE_SET_FLAG_ISEXCLUDED(excluded_objects,   NQueue+1+NSemaphore+1+NMutex+1+NMem+1+NTimer+1+NEvent+1+taskIndex)
#define TRACE_CLEAR_TASK_FLAG_ISEXCLUDED(taskIndex) TRACE_CLEAR_FLAG_ISEXCLUDED(excluded_objects, NQueue+1+NSemaphore+1+NMutex+1+NMem+1+NTimer+1+NEvent+1+taskIndex)
#define TRACE_GET_TASK_FLAG_ISEXCLUDED(taskIndex)   TRACE_GET_FLAG_ISEXCLUDED(excluded_objects,   NQueue+1+NSemaphore+1+NMutex+1+NMem+1+NTimer+1+NEvent+1+taskIndex)
...
case TRACE_CLASS_EVENT: \
    TRACE_CLEAR_EVENT_FLAG_ISEXCLUDED(handle); \
    break; \
...
case TRACE_CLASS_TIMER: \
    ...
    break; \
case TRACE_CLASS_EVENT: \
    TRACE_SET_EVENT_FLAG_ISEXCLUDED(handle); \
    break; \
case TRACE_CLASS_TASK: \
...
#define TRACE_TASK_SWITCH(from, to)     traceTASK_SWITCHED_IN(to)
#undef  TRACE_TASK_CREATE
#define TRACE_TASK_CREATE(task)         traceTASK_CREATE(task)
#undef  TRACE_TASK_SLEEP
#define TRACE_TASK_SLEEP(task, ticks)   traceTASK_DELAY(ticks)
#undef  TRACE_INTRPT_TASK_SWITCH
#define TRACE_INTRPT_TASK_SWITCH(from, to)  traceTASK_SWITCHED_IN(to)
#undef  TRACE_TASK_PRI_CHANGE
#define TRACE_TASK_PRI_CHANGE(task, task_pri_chg, pri)  traceTASK_PRIORITY_SET(task_pri_chg, pri)
#undef  TRACE_TASK_SUSPEND
#define TRACE_TASK_SUSPEND(task, task_suspended) traceTASK_SUSPEND(task_suspended)
#undef  TRACE_TASK_RESUME
#define TRACE_TASK_RESUME(task, task_resumed)   traceTASK_RESUME(task_resumed)
#undef  TRACE_TASK_DEL
#define TRACE_TASK_DEL(task, task_del)          traceTASK_DELETE(task_del)
#undef  TRACE_TASK_WAIT_ABORT
#define TRACE_TASK_WAIT_ABORT(task, task_abort)

/* semaphore trace */
#undef  TRACE_SEM_CREATE
#define TRACE_SEM_CREATE(task, sem)     traceSEM_CREATE(SEM, sem, "sem")
#undef  TRACE_SEM_OVERFLOW
#define TRACE_SEM_OVERFLOW(task, sem)   traceSEM_SEND_FAILED(SEM, sem)
#undef  TRACE_SEM_CNT_INCREASE
#define TRACE_SEM_CNT_INCREASE(task, sem)   traceSEM_SEND(SEM, sem)
#undef  TRACE_SEM_GET_SUCCESS
#define TRACE_SEM_GET_SUCCESS(task, sem)    traceSEM_RECEIVE(SEM, sem);
#undef  TRACE_SEM_GET_BLK
#define TRACE_SEM_GET_BLK(task, sem, wait_option)   traceSEM_RECEIVE_BLOCK(SEM, sem)

#undef  TRACE_SEM_TASK_WAKE
#define TRACE_SEM_TASK_WAKE(task, task_waked_up, sem, opt_wake_all)
#undef  TRACE_SEM_DEL
#define TRACE_SEM_DEL(task, sem)        traceSEM_DELETE(SEM, sem)

/* mutex trace */
#undef  TRACE_MUTEX_CREATE
#define TRACE_MUTEX_CREATE(task, mutex, name)       traceMUTEX_CREATE(MUTEX, mutex, "Mutex")
#undef  TRACE_MUTEX_RELEASE
#define TRACE_MUTEX_RELEASE(task, task_release, new_pri)
#undef  TRACE_MUTEX_GET
#define TRACE_MUTEX_GET(task, mutex, wait_option)   traceMUTEX_RECEIVE(MUTEX, mutex)
#undef  TRACE_TASK_PRI_INV
#define TRACE_TASK_PRI_INV(task, mtxtsk)            traceTASK_PRIORITY_INHERIT(mtxtsk, mtxtsk->prio)
#undef  TRACE_MUTEX_GET_BLK
#define TRACE_MUTEX_GET_BLK(task, mutex, wait_option)   traceMUTEX_RECEIVE_BLOCK(MUTEX, mutex)
#undef  TRACE_MUTEX_RELEASE_SUCCESS
#define TRACE_MUTEX_RELEASE_SUCCESS(task, mutex)        traceMUTEX_SEND(MUTEX, mutex)
#undef  TRACE_MUTEX_TASK_WAKE
#define TRACE_MUTEX_TASK_WAKE(task, task_waked_up, mutex)
#undef  TRACE_MUTEX_DEL
#define TRACE_MUTEX_DEL(task, mutex)                traceMUTEX_DELETE(MUTEX, mutex)

/* event trace */
#undef TRACE_EVENT_CREATE
#define TRACE_EVENT_CREATE(task, event, name, flags_init)   traceEVENT_GROUP_CREATE(EVENT_GROUP, event, "Event")
#undef  TRACE_EVENT_GET
#define TRACE_EVENT_GET(task, event)                        traceEVENT_GROUP_RECEIVE(EVENT_GROUP, event)
#undef  TRACE_EVENT_GET_BLK
#define TRACE_EVENT_GET_BLK(task, event, wait_option)       traceEVENT_GROUP_RECEIVE_BLOCK(EVENT_GROUP, event)
#undef  TRACE_EVENT_TASK_WAKE
#define TRACE_EVENT_TASK_WAKE(task, task_waked_up, event)
#undef  TRACE_EVENT_DEL
#define TRACE_EVENT_DEL(task, event)                        traceEVENT_GROUP_DELETE(EVENT_GROUP, event)

/* buf_queue trace */
#undef  TRACE_BUF_QUEUE_CREATE
#define TRACE_BUF_QUEUE_CREATE(task, buf_queue)     //traceQ_CREATE(Q, buf_queue, "q")
#undef  TRACE_BUF_QUEUE_MAX
#define TRACE_BUF_QUEUE_MAX(task, buf_queue, msg, msg_size)
#undef  TRACE_BUF_QUEUE_POST
#define TRACE_BUF_QUEUE_POST(task, buf_queue, msg, msg_size)    //traceQ_SEND(Q, buf_queue)
#undef  TRACE_BUF_QUEUE_TASK_WAKE
#define TRACE_BUF_QUEUE_TASK_WAKE(task, task_waked_up, queue)
#undef  TRACE_BUF_QUEUE_GET_BLK
#define TRACE_BUF_QUEUE_GET_BLK(task, buf_queue, wait_option) //traceQ_RECEIVE_BLOCK(Q, buf_queue)

/* timer trace */
#undef  TRACE_TIMER_CREATE
#define TRACE_TIMER_CREATE(task, timer)             traceTIMER_CREATE(TIMER, timer, "Timer")
#undef  TRACE_TIMER_DEL
#define TRACE_TIMER_DEL(task, timer)                traceTIMER_DELETE(TIMER, timer)
...
#define TRACE_WORK_INIT(task, work)             traceWQ_INIT(WQ, work, "wq")
#undef  TRACE_WORKQUEUE_CREATE
#define TRACE_WORKQUEUE_CREATE(task, workqueue) traceWQ_CREATE(WQ, workqueue, "wq")
#undef  TRACE_WORKQUEUE_DEL
#define TRACE_WORKQUEUE_DEL(task, workqueue)    traceWQ_DELETE(WQ, workqueue)
