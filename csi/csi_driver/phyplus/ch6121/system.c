...
void hal_init(void)
{
    ...
    hal_ret_sram_enable(RET_SRAM0 |RET_SRAM2);

#ifdef CONFIG_RTC_CLK_XTAL
...
__attribute__((weak)) void SystemInit(void)
{
    ...
    JUMP_FUNCTION(HARDFAULT_HANDLER) = (uint32_t)hard_fault;
    JUMP_FUNCTION(PENDSV_HANDLER) = (uint32_t)PendSV_Handler;
    JUMP_FUNCTION(SYSTICK_HANDLER) = (uint32_t)CSI_SysTick_Handler;
