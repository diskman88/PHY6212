...
#ifdef ENABLE_CACHE
static uint16_t g_kv_cached_size = INIT_CACHE_NUM;
void kv_set_cached_size(int size)
{
    if (size)
    {
        g_kv_cached_size = size;
    }
}
...
int kv_cache_in(kv_t *kv, const char *key, int block_id, uint32_t offset)
{
    ...
    if (i >= kv->cache_num) {
        cache_node_t *old_cache = kv->cache;
    ...
    //printf("kv_cache_in: key %s, hash %d, block id %d, offset %d add in\n", key, hash, block_id, offset);
    cache->hash     = hash;
...
static int _iter_cache_node(kvnode_t *node, void *p)
{
    ...
    if (NODE_VAILD(node))
    {
        ret = kv_cache_in(kv, (const char *)KVNODE_OFFSET2CACHE(node, head_offset), (int16_t)node->block->id, (uint16_t)node->head_offset);
        if (ret)
        {
            return ret;
        }
    ...
    while (1) {
        if (kv->blocks[kv->bid].ro_count == 0 && kv->bid != kv->gc_bid) {
            ...
            if (offset >= 0) {
                ...
                int ret =kv_cache_in(kv, key, kv->bid, offset);
                if (ret)
                {
                    return ret;
                }
