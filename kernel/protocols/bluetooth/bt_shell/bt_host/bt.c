...
static int cmd_wl_add(int argc, char *argv[])
{
    ...
    return ble_stack_white_list_add(&addr);
}
static int cmd_wl_remove(int argc, char *argv[])
{
    ...
    return ble_stack_white_list_remove(&addr);
}
static int cmd_wl_clear(int argc, char *argv[])
{
    printf("Clear white list\n");
    return ble_stack_white_list_clear();
}
...
static const struct shell_cmd bt_commands[] = {
    ...
    },
    {
        ...
        "<type 0:server 1 client> <mode 0:loop 1:single> <server tx mode 0:notify 1:indicata> <server rx handle> <client tx mode 0:write 1:write_withoutresponse> "\
