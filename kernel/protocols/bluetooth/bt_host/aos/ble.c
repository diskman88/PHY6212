...
static void connected(struct bt_conn *conn, u8_t err)
{
    ...
    event_data.conn_handle = bt_conn_get_id(conn);
...
int ble_stack_adv_start(adv_param_t *param)
{
    ...
    if (((param->ad == NULL && param->ad_num != 0) || (param->ad != NULL && param->ad_num == 0))
    ...
    if (param->type == ADV_DIRECT_IND) {
        p.options |= BT_LE_ADV_OPT_DIRECT;
    }

    if (param->type == ADV_DIRECT_IND_LOW_DUTY) {
        p.options |= BT_LE_ADV_OPT_DIRECT_LOW_DUTY;
    }
    ...
    p.chan_map = param->channel_map;
    p.filter_policy = param->filter_policy;
    p.peer_addr = *(bt_addr_le_t *)&param->direct_peer_addr;

    if (param->type == ADV_DIRECT_IND_LOW_DUTY || param->type == ADV_DIRECT_IND) {
        if (!bt_addr_le_cmp(&p.peer_addr, BT_ADDR_LE_ANY)) {
            return -BLE_STACK_ERR_PARAM;
        }
    ...
    ret = bt_le_adv_start(&p, (struct bt_data *)param->ad, param->ad_num,
...
int ble_stack_scan_start(const scan_param_t *param)
{
    ...
    p.scan_filter = param->scan_filter;
...
int ble_stack_gatt_notificate(int16_t conn_handle, uint16_t char_handle, const uint8_t *data, uint16_t len)
{
    ...
    conn = bt_conn_lookup_id(conn_handle);
...
void key_miss_cb()
{
   ble_stack_event_callback(EVENT_SMP_KEY_MISS, NULL, 0);
}
                          ...
                          struct bt_gatt_discover_params *params)
{
    ...
    if (attr == NULL) {
        ...
        event_data.comp.conn_handle = bt_conn_get_id(conn);
    ...
    if (params->type == BT_GATT_DISCOVER_PRIMARY) {
        ...
        event_data.svc.conn_handle = bt_conn_get_id(conn);
        ...
        event_data.svc_inc.conn_handle = bt_conn_get_id(conn);
        ...
        event_data.char_c.conn_handle = bt_conn_get_id(conn);
...
int ble_stack_gatt_write(int16_t conn_handle, uint16_t attr_handle, uint8_t *data, uint16_t len, uint16_t offset, gatt_write_en type)
{
    ...
    struct bt_conn *conn = bt_conn_lookup_id(conn_handle);
...
int ble_stack_connect(dev_addr_t *peer_addr, conn_param_t *param, uint8_t auto_connect)
{
    ...
    if (conn) {
        conn_handle = bt_conn_get_id(conn);
        bt_conn_unref(conn);
        return conn_handle;
    }
...
int ble_stack_pref_phy_set(int16_t conn_handle, pref_phy_en pref_tx_phy, pref_phy_en pref_rx_phy)
{
    ...
    return hci_api_le_set_phy(conn_handle, 0, pref_tx_phy, pref_rx_phy,BT_HCI_LE_PHY_CODED_ANY);
}
...
{
    return hci_api_white_list_clear();
}

int ble_stack_white_list_add(dev_addr_t *peer_addr)
{
    return hci_api_white_list_add(peer_addr->type, peer_addr->val);
}

int ble_stack_white_list_remove(dev_addr_t *peer_addr)
{
    return hci_api_white_list_remove(peer_addr->type, peer_addr->val);
}
...
int ble_stack_white_list_size()
{
    uint8_t size = 0;
    int ret;
    ret = hci_api_white_list_size(&size);
...
static void _paried_key_find(struct bt_keys *keys, void *data)
{
    dev_addr_t peer_addr;
    struct dev_addr_callback_func *call = (struct dev_addr_callback_func *)data;

    if (keys) {
        peer_addr.type = keys->addr.type;
        memcpy(peer_addr.val, keys->addr.a.val, sizeof(peer_addr.val));
...
int ble_stack_paired_dev_foreach(void (*func)(dev_addr_t *addr, void *data), void *data)
{
    ...
    bt_keys_foreach(BT_KEYS_ALL, _paried_key_find, &inpara);
    return BLE_STACK_OK;
}
