...
static u8_t att_mtu_req(struct bt_att *att, struct net_buf *buf)
{
	...
	extern void mtu_exchange_cb(struct bt_conn *conn, u8_t err,
...
static int att_send_req(struct bt_att *att, struct bt_att_req *req)
{
	...
	g_hci_debug_counter.att_out_count++;
...
static u8_t att_handle_read_group_type_rsp(struct bt_att *att, struct net_buf *buf)
{
	BT_DBG("");
...
static const struct att_handler {
	...
	{ BT_ATT_OP_READ_GROUP_RSP,
		...
		ATT_RESPONSE,
		att_handle_read_group_type_rsp },
	{ BT_ATT_OP_WRITE_REQ,
...
static void bt_att_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	g_hci_debug_counter.att_in_count++;
...
static void bt_att_disconnected(struct bt_l2cap_chan *chan)
{
	...
	bt_gatt_disconnected(ch->chan.conn);
	k_delayed_work_cancel(&att->timeout_work);
	memset(&att->chan, 0, sizeof(att->chan));
	memset(&att->flags, 0, sizeof(att->flags));
	memset(&att->reqs, 0, sizeof(att->reqs));
	k_sem_delete(&att->tx_sem);
	att->req = NULL;
}
...
int bt_att_send(struct bt_conn *conn, struct net_buf *buf)
{
	...
	g_hci_debug_counter.att_out_count++;
	err = bt_l2cap_send_cb(conn, BT_L2CAP_CID_ATT, buf, att_cb(buf));
    if (err)
    {
        k_sem_give(&att->tx_sem);
    }
