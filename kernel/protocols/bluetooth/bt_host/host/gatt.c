...
static ssize_t read_ppcp(struct bt_conn *conn,
                         const struct bt_gatt_attr *attr, void *buf,
                         u16_t len, u16_t offset)
{
                          ...
                          const void *value, u16_t value_len)
{
    ...
    memcpy(buf, (const u8_t *)value + offset, len);
                       ...
                       size_t len)
{
    ...
    return bt_l2cap_send(conn, BT_L2CAP_CID_ATT, buf);
}
                          ...
                          const void *data, u16_t length)
{
    ...
    ble_stack_gatt_notify_cb(bt_conn_get_id(conn), handle, data, length);
...
static void gatt_read_group_type_rsp(struct bt_conn *conn, u8_t err,
                                     const void *pdu, u16_t length,
                                     void *user_data)
{
    ...
    attr_val_len = rsp->len - sizeof(struct bt_att_group_data);
    length -= 1;
    pdu = (u8_t *)pdu + 1;

    /* Parse attributes found */
    for (i = 0; length >= rsp->len;
         i++, length -= rsp->len, pdu = (u8_t *)pdu + rsp->len) {
        struct bt_gatt_attr attr = {0};
        struct bt_gatt_service_val value;
        const struct bt_att_group_data *group_data = pdu;
        start_handle = sys_le16_to_cpu(group_data->start_handle);
        end_handle = sys_le16_to_cpu(group_data->end_handle);
        ...
        memcpy(&attr_value.uuid16, group_data->value, attr_val_len);

        if (attr_val_len == 2) {
            u.uuid.type = BT_UUID_TYPE_16;
            u.u16.val = sys_le16_to_cpu(attr_value.uuid16);
        } else if (attr_val_len == 16) {
            u.uuid.type = BT_UUID_TYPE_128;
            memcpy(u.u128.val, attr_value.uuid128, 16);
        }
...
static int gatt_read_group_type(struct bt_conn *conn,
                                struct bt_gatt_discover_params *params)
{
    struct net_buf *buf;
    struct bt_att_read_group_req *req;
    struct bt_uuid *uuid = BT_UUID_GATT_PRIMARY;
    ...
    // req->uuid = sys_cpu_to_le16(BT_UUID_16(uuid)->val);

    BT_DBG("uuid %s start_handle 0x%04x end_handle 0x%04x",
           bt_uuid_str(uuid), params->start_handle,
           params->end_handle);

    net_buf_add_le16(buf, BT_UUID_16(uuid)->val);

    return gatt_send(conn, buf, gatt_read_group_type_rsp, params, NULL);
}
                           ...
                           u16_t length)
{
    ...
    for (length--, pdu = rsp->data; length >= rsp->len;
         length -= rsp->len, pdu = (u8_t *)pdu + rsp->len) {
        struct bt_gatt_attr *attr;
    ...
    switch (params->type) {
        case BT_GATT_DISCOVER_PRIMARY:
            if (params->uuid) {
                ...
                return gatt_read_group_type(conn, params);
            }
                                   ...
                                   void *user_data)
{
    ...
    /*the params->func may release params,so check if it equals NULL before use*/
    /* mark read as complete since read multiple is single response */
    if(NULL != params && params->func){
        params->func(conn, 0, params, NULL, 0);
    }
}
...
{
    struct ccc_save save = {0};
    char val[BT_SETTINGS_SIZE(sizeof(save.store))] = {0};
