                    ...
                    bt_conn_tx_cb_t cb)
{
    ...
    return bt_conn_process_tx(conn);
                      ...
                      bool always_consume)
{
    ...
    if (err) {
        ...
        if (err == BT_HCI_ERR_MEM_CAPACITY_EXCEEDED)
        {
            err = -ENOMEM;
        }
        else
...
static void conn_cleanup(struct bt_conn *conn)
{
    ...
    //k_fifo_del(&conn->tx_queue);
    //k_fifo_del(&conn->tx_notify);
                            ...
                            const struct bt_le_conn_param *param)
{
    ...
    if ((param->interval_min < BT_LE_CONN_INT_MIN)
        || (param->interval_min > BT_LE_CONN_INT_MAX)
        || (param->interval_max < BT_LE_CONN_INT_MIN)
        || (param->interval_max > BT_LE_CONN_INT_MAX)
        || (param->interval_max < param->interval_min)
        || (param->latency > BT_LE_CONN_LAT_MAX)
        || (param->timeout < BT_LE_CONN_TO_MIN)
        || (param->timeout > BT_LE_CONN_TO_MAX))
    {
        return -EINVAL;
    }
    if (!bt_le_conn_params_valid(param)) {
...
int bt_conn_le_conn_update(struct bt_conn *conn,
                           const struct bt_le_conn_param *param)
{
    return hci_api_le_conn_updata(conn->handle, param->interval_min, param->interval_max, 
