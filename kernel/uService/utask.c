...
static void utask_entry(void *data)
{
    ...
    while (task->running) {
        if (aos_queue_recv(&task->queue, AOS_WAIT_FOREVER, &rpc, &size) == 0) {
...
utask_t *utask_new(const char *name, size_t stack_size, int queue_count, int prio)
{
    ...
    if (aos_task_new_ext(&task->task, name ? name : "utask", utask_entry, task, NULL, stack_size, prio) != 0) {
        goto out3;
    }
...
utask_t *utask_new_ext(utask_t *task, const char *name, void *stack_buf, size_t stack_size, void *queue_buf, int queue_length, int prio)
{
    if (task == NULL || stack_buf == NULL || stack_size <= 0 || queue_buf == NULL || queue_length <= 0 ) {
        return NULL;
    }
    int queue_buffer_size = queue_length * sizeof(rpc_t);
    ...
    task->queue_count = queue_length;
    task->queue_buffer = queue_buf;
    ...
    if (aos_task_new_ext(&task->task, name ? name : "utask", utask_entry, task, stack_buf, stack_size, prio) != 0) {
        goto out3;
    }
...
void utask_join(utask_t *task)
{
    ...
    rpc.srv    = NULL;
    rpc.cmd_id = CMD_TASK_QUIT;

    task->running = 0;
    aos_queue_send(&task->queue, &rpc, sizeof(rpc));
