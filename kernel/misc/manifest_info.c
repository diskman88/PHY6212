...
static int mtb_find_addr(mtb_t *mtb)
{
    ...
    while(flash_addr < flash_end) {
        if (!flash_read(flash_dev, flash_addr, (uint8_t *)&m_buf, sizeof(mtb_head_t))) {
            if (m_buf.head.magic == BMTB_MAGIC) {
                ...
                if (m_buf.head.version == 1) {
                    if (flash_addr + m_buf.head.size < flash_end) {
                        mtb_head_t i_buf;
                        if (!flash_read(flash_dev, flash_addr + m_buf.head.size, (uint8_t *)&i_buf, sizeof(mtb_head_t))) {
    ...
    if (flash_read(flash_dev, flash_addr + bmtb_size, (uint8_t *)g_manfest_tb_ram, imtb_size)) {
        goto fail;
    }
...
static int partition_info_init(mtb_t *mtb, uint8_t *buf, uint32_t *buf_size)
{
    ...
    if (size > *buf_size) {
        LOGD(TAG, "partition init size overflow %d!!!", size);
        return -1;
    }
    *buf_size = size;
    return 0;
