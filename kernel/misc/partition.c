...
#define PARTITIONS_NUM 9

static struct {
    hal_logic_partition_t *scn_list;
    uint8_t num;
} g_partion_array;

static hal_logic_partition_t scn_list_num[PARTITIONS_NUM];
...
//        aos_free(buf);
    ...
    if (num > PARTITIONS_NUM) {
        LOGE(TAG, "too many partitions %d", num);
        return -1;
    }
    g_partion_array.scn_list = scn_list_num;
    //g_partion_array.scn_list = (hal_logic_partition_t *)aos_zalloc(sizeof(hal_logic_partition_t) * num);
        ...
        if (temp_flash_dev != NULL) {
            ...
            for (i = 0; i < num; i++) {
                if ((cur_flash_info.start_addr <= part_info[i].part_addr) &&
                    ...
                    hal_logic_partition_t *scn = &g_partion_array.scn_list[i];
...
{
    hal_logic_partition_t *node = hal_flash_get_info(partition);
    if (node != NULL) {
