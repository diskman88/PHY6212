...
static struct ota_rx_t ota_rx __attribute__((section("noretention_mem_area0"))) = {0};
...
static inline int ota_recv_data(const uint8_t *data, uint16_t len)
{
    ...
    /* the last data */
    if (len == cp_size + 2 && data[len - 2] == OTA_PACKET_END && data[len - 1] == OTA_PACKET_START
        && ota_rx.rx_len == ota_rx.expect_len) {
    ...
    } else if (ota_rx.rx_len >= ota_rx.expect_len) {
        LOGE(TAG, "ota packet err %d, rx len %d expect len %d", ota_ctrl.expect_packet_count, ota_rx.rx_len, ota_rx.expect_len);
        ota_status_change(OTA_ST_REQUEST);
    }
