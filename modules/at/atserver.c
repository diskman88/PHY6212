...
atserver_uservice_t g_atserver;
...
{
    if (atmode(g_atserver).mode == PASS_THROUGH_MODE && atmode(g_atserver).cb != NULL) {
        atmode(g_atserver).cb(g_atserver.uart_dev);
        return 0;
    }
...
int atserver_rawdata_send(const void *buf, uint32_t len)
{
    aos_check_return_einval(buf && len);

    return uart_send(g_atserver.uart_dev, buf, len);
}
...
int atserver_passthrough_cb_register(passthrough_cb cb)
{
...
int atserver_exit_passthrough(void)
{
    atserver_mode_t atmode;

    atmode.mode = NORMAL_MODE;
    atmode.cb   = NULL;
