...
int at_ble_conf_load(at_conf_load *conf_load)
{
    ...
    //name
    ret = aos_kv_getstring("BT_NAME", (char *)conf_load->bt_name, 28 + 1);
    ...
    if (ret != 0 || (conf_load->baud != 9600 && conf_load->baud != 115200 && conf_load->baud != 250000 && conf_load->baud != 512000)) {
        conf_load->baud = BT_TX_BAUD_DEF;
    }

    //CONN_UPDATE_FLAG
    ret = aos_kv_get("CONN_UPDATE_ON", &conf_load->conn_update_def_on, &length);

    if (ret != 0) {
        conf_load->conn_update_def_on = CONN_UPDATE_DEF_ON_OFF;
    }

    //CONN PRAM
    if (conf_load->conn_update_def_on) {
        length = sizeof(conn_param_t);
        ret  = aos_kv_get("CONN_PARAM", &conf_load->conn_param, &length);
