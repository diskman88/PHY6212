...
void at_cmd_bt_name(char *cmd, int type, char *data)
{
    if (!ble_server.init_flag) {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_AT_SERVICE_NOT_INITIALIZED));
        return;
    }
    ...
    if (type == WRITE_CMD) {
        ...
        if (len > 0 && len <= DEV_NAME_MAX_LENGTH) {
            ...
            if (!ret) {
                ...
                AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_BT_KV_SAVE_PARAM));
            }
        } else {
            AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
        }
...
static void dump_adv_data(adv_param_t param)
{
    extern char *bt_hex(const void *buf, size_t len);
    uint8_t adv_on = 0;

    if (llState == LL_STATE_ADV_DIRECTED || llState == LL_STATE_ADV_NONCONN || llState == LL_STATE_ADV_UNDIRECTED) {
        adv_on = 1;
    }

    atserver_send("+BTADV:%d,%d,%d,%d", adv_on, param.type, param.interval_min, param.interval_max);

    if (param.ad_num) {
        atserver_send(",");

        for (int i = 0; i < param.ad_num; i++) {
            atserver_send("%02x%02x%s", param.ad[i].len + 1, param.ad[i].type, bt_hex(param.ad[i].data, param.ad[i].len));
        }
    } else {
        atserver_send("NULL");
    }
    ...
    if (param.sd_num) {
        ...
        for (int i = 0; i < param.sd_num; i++) {
            atserver_send("%02x%02x%s", param.sd[i].len + 1, param.sd[i].type, bt_hex(param.sd[i].data, param.sd[i].len));
        }
    } else {
        atserver_send("NULL");
    }
...
void at_cmd_bt_adv(char *cmd, int type, char *data)
{
    ...
    if (ble_server.at_config.role == MASTER && (ble_server.temp_role_set != SLAVE || type == READ_CMD)) {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_CMD_NOT_SUPPORTED));
        return;
    }
    ...
    if (type == WRITE_CMD) {
        ...
        if (input_num >= 2) {
            ...
            if (adv_type != ADV_IND && adv_type != ADV_DIRECT_IND && adv_type != ADV_NONCONN_IND) {
                goto FAILD;
            } else {
        ...
        if (ble_server.temp_adv_set.interval_min > ble_server.temp_adv_set.interval_max) {
            goto FAILD;
        }
...
FAILD:
        ...
        if (set_adv_flag) {
            dump_adv_data(ble_server.temp_adv_set);
        } else {
            dump_adv_data(AT_BLE_UART_CONFIG_SLAVE(ble_server).param);
        }
    } else if (type == TEST_CMD) {
        AT_BACK_STR("\r\nAT+BTADV=<on_off>,[type],[interval_min],[interval_max],[ad],[sd]");
    } else {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
    }
}
...
void at_cmd_bt_adv_def(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        AT_BACK_RET_OK_INT(cmd, AT_BLE_UART_CONFIG_SLAVE(ble_server).adv_def_on);
    } else if (type == TEST_CMD) {
        AT_BACK_STR("\r\nAT+BTAUTOADV=<def_on>");
    } else {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
...
void at_cmd_bt_conn(char *cmd, int type, char *data)
{
    ...
    if (ble_server.at_config.role == SLAVE && (ble_server.temp_role_set != MASTER || type == READ_CMD)) {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_CMD_NOT_SUPPORTED));
        return;
    }
    ...
    if (type == WRITE_CMD) {
        ...
        dev.type = addr_type;
        /*
        if (addr_type == DEV_ADDR_LE_RANDOM && ((dev.val[5] & 0XC0) != 0xC0)) {
            AT_BACK_RET_ERR(cmd,ERR_AT(AT_ERR_INVAL));
            return;
        }*/
    } else if (type == TEST_CMD) {
...
void at_cmd_bt_conn_info_get(char *cmd, int type, char *data)
{
    if (!ble_server.init_flag || ble_server.recv_cb.at_mode_recv_cb == NULL) {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_AT_SERVICE_NOT_INITIALIZED));
        return;
    ...
    if (type == EXECUTE_CMD) {
        ret = ble_server.recv_cb.at_mode_recv_cb(AT_BT_CONN_INFO_GET, NULL, 0, NULL);

        if (ret < 0) {
            AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_BT_GET_CONN_INFO_FAILD));
        } else {
            AT_BACK_OK();
        }
    } else {
...
void at_cmd_bt_conn_def(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        AT_BACK_RET_OK_INT(cmd, AT_BLE_UART_CONFIG_MASTER(ble_server).conn_def_on);
    } else if (type == TEST_CMD) {
        AT_BACK_STR("\r\nAT+BTAUTOCONN=<def_on>");
    } else {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
    }
...
{
    if (err) {
        AT_BACK_RET_ERR("AT+BTTX:", ERR_AT(AT_ERR_BT_TX_FAILED));
    } else {
        AT_BACK_OK();
    }
}
...
void at_cmd_bt_tx(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        if (data_len == 0 || data_len > TX_MAX_LEN) {
            AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_BT_TX_LEN));
        } else {
            ret = ble_server.recv_cb.at_mode_recv_cb(AT_BT_TX_DATA, data, data_len, &tx_cb);

            if (ret < 0) {
                AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_BT_TX_FAILED));
            }
        }
    } else if (type == TEST_CMD) {
        AT_BACK_STR("\r\nAT+BTTX=<data>");
...
void at_cmd_bt_role(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        AT_BACK_RET_OK_INT(cmd, ble_server.at_config.role);
    } else if (type == TEST_CMD) {
        AT_BACK_STR("\r\nAT+BTROLE=<role>");
    } else {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
...
{
    if (!ble_server.init_flag) {
        AT_BACK_RET_OK_INT(cmd, ERR_AT(AT_ERR_AT_SERVICE_NOT_INITIALIZED));
        return;
    }
...
void at_cmd_bt_sleep_mode(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        AT_BACK_RET_OK_INT(cmd, ble_server.at_config.sleep_mode);
    } else if (type == TEST_CMD) {
        AT_BACK_STR("\r\nAT+BTSLEEP=<mode>");
    } else {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
...
void at_cmd_bt_tx_power(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        AT_BACK_RET_OK_INT(cmd, tx_power);
    } else if (type == TEST_CMD) {
        AT_BACK_STR("\r\nAT+BTTXPOW=<tx_power>");
    } else {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
...
int read_public_addr(dev_addr_t *addr)
{
    ...
    addr->val[0] = temp_data[3];
    addr->val[1] = temp_data[2];
    addr->val[2] = temp_data[1];
    addr->val[3] = temp_data[0];
    addr->val[4] = temp_data[5];
    addr->val[5] = temp_data[4];
    addr->type = 0;
...
_STATIC int ewrite_flash(partition_t handle, const uint8_t *buffer, int length, int offset)
{
    hal_logic_partition_t *lp;
...
{
    uint8_t  temp_data[6];
    partition_t handle = -1;
    handle = partition_open("FCDS");
    temp_data[0] = addr.val[3];
    temp_data[1] = addr.val[2];
    temp_data[2] = addr.val[1];
    temp_data[3] = addr.val[0];
    temp_data[4] = addr.val[5];
    temp_data[5] = addr.val[4];
...
void at_cmd_bt_baud(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        if (baud != 9600 && baud != 115200 && baud != 250000 && baud != 512000) {
            AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
            return;
        }
        ...
        AT_BACK_RET_OK_INT(cmd, ble_server.at_config.baud);
    } else if (type == TEST_CMD) {
        AT_BACK_STR("\r\nAT+BAUD=<baud>");
    } else {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
...
void at_cmd_bt_find(char *cmd, int type, char *data)
{
    ...
    if (type == EXECUTE_CMD) {
        ...
        if (ret < 0) {
            if (-1 == ret) {
                AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_BT_FIND_NO_DEV));
            } else {
                AT_BACK_RET_ERR(cmd, ERR_AT(ret));
            }
...
void at_cmd_bt_conn_update(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        AT_BACK_STR("\r\nAT+BTCONNUPDATE=<def_on>,<interval_min>,<interval_max>,<lantency>,<timeout>");
    } else {
        AT_BACK_RET_ERR(cmd, ERR_AT(AT_ERR_INVAL));
    }
}

void at_ble_uartmode_recv(dev_t *dev)
{
    int ret;
