...
void at_model_cb(mesh_model_event_en event, void *p_arg)
{
    ...
    switch (event) {
        ...
        case BT_MESH_MODEL_VENDOR_MESSAGES: {
            ...
            atserver_send("\r\n+MESHTRS:%04x,%02x,%s\r\n", unicast_addr, len, bt_hex(data, len));
        }
        break;
...
void at_cmd_btmesh_rst_dev(char *cmd, int type, char *data)
{
    ...
    if (type == EXECUTE_CMD) {
        ...
        extern hal_logic_partition_t *hal_flash_get_info(partition_t partition);
        hal_logic_partition_t *node = hal_flash_get_info(h);
        uint8_t block_count = node->length / node->sector_size;
        ret = partition_erase(h, 0, block_count);

        if (ret) {
            LOGD(TAG, "kv erase faild\n");
        } else {
            LOGD(TAG, "kv erase succeed block num :%d\n", block_count);
        }
...
void at_cmd_btmesh_prov_netkey_get(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        if (!key_val) {
            AT_BACK_RET_ERR(cmd, ERR_MESH_AT(AT_ERR_MESH_NETKEY_GET_FAILD));
            return;
        }
...
void at_cmd_btmesh_prov_appkey_get(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        if (!key_val) {
            AT_BACK_RET_ERR(cmd, ERR_MESH_AT(AT_ERR_MESH_APPKEY_GET_FAILD));
            return;
        }
...
void at_cmd_btmesh_prov_add_node(char *cmd, int type, char *data)
{
    ...
    if (type == WRITE_CMD) {
        ...
        ret =  provisioner_add_node_manu(&node, dev_key);
