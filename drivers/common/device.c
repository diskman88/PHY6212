...
#define DEVICE_MAX_SIZE (128 + sizeof(dev_t))
#define DEVICE_FREE 0
#define DEVICE_USED 1

static uint32_t device_list[CONFIG_MAX_DEV_COUNT][DEVICE_MAX_SIZE / 4] = {0};

dev_t *device_new(driver_t *drv, int size, int id)
{
    dev_t *dev = NULL; //(dev_t*)aos_zalloc(size);
    int i = 0;
...
int device_lock(dev_t *dev)
{
    return aos_mutex_lock(&dev->mutex, AOS_WAIT_FOREVER);
}

int device_unlock(dev_t *dev)
{
...
int driver_register(driver_t *drv, void *config, int idx)
{
    ...
    dev_t *dev = drv->init(drv, config, idx);
...
dev_t *device_find(const char *name, int id)
{
    dev_t *node;
    int i;
    LIST_LOCK();
...
dev_t *device_open(const char *name)
{
    int id = 0;
    char buffer[12] = {0};
...
dev_t *device_open_id(const char *name, int id)
{
    dev_t *dev = device_find(name, id);
...
int device_valid(dev_t *dev, const char *name)
{
    if (dev && dev->drv) {
        const char *n = NULL;
        driver_t *drv = (driver_t*)dev->drv;
