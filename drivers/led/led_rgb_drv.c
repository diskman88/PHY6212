...
static dev_t *yoc_led_rgb_init(driver_t *drv, void *config, int id)
{
...
static int yoc_led_rgb_lpm(dev_t *dev, int state)
{
    return 0;
}

static void yoc_led_timer_trigger(dev_t *dev)
{
...
static void yoc_led_delay_action(void *timer, void *args)
{
    ...
    if (led->blink == BLINK_ON) {
        ...
        LED_GPIO_SET(led->led_r_pin_handle, 1, led_config->flip);
        LED_GPIO_SET(led->led_g_pin_handle, 1, led_config->flip);
        LED_GPIO_SET(led->led_b_pin_handle, 1, led_config->flip);
    }

    led->ctrl_req = 0;
    yoc_led_timer_trigger((dev_t *)led);
}

static int yoc_led_rgb_config(dev_t *dev, uint32_t r_pin, uint32_t g_pin, uint32_t b_pin)
{
    led_dev_t *       led        = (led_dev_t *)dev;
    led_pin_config_t *led_config = (led_pin_config_t *)led->device.config;
    int               ret        = 0;
...
static int yoc_led_rgb_open(dev_t *dev)
{
    led_dev_t *       led        = (led_dev_t *)dev;
    led_pin_config_t *led_config = (led_pin_config_t *)led->device.config;
...
static int yoc_led_rgb_close(dev_t *dev)
{
    led_dev_t *       led        = (led_dev_t *)dev;
    led_pin_config_t *led_config = (led_pin_config_t *)led->device.config;
...
static int yoc_led_rgb_control(dev_t *dev, int color, int on_time, int off_time)
{
    led_dev_t *       led        = (led_dev_t *)dev;
    led_pin_config_t *led_config = (led_pin_config_t *)led->device.config;
