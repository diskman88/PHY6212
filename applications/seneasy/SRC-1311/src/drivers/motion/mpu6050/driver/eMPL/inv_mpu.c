...
#define EMPL_TARGET_PHY6212 CONFIG_EMPL_TARGET_PHY6212
#define MPU6050 CONFIG_MOTION_MPU6050
...
#elif defined EMPL_TARGET_PHY6212  
#include <aos/kernel.h>
#include <csi_kernel.h>
#include <aos/log.h>

static int Sensors_I2C_WriteRegister(uint16_t addr, uint8_t reg, uint16_t len, uint8_t *data){
    return 0;
}

static int Sensors_I2C_ReadRegister(uint16_t addr, uint8_t reg, uint16_t len, uint8_t *data){
    return 0;
}
...
static void get_ticks(unsigned long *ticks){
    *ticks = (unsigned long)csi_kernel_get_ticks();
}

#define LOG_I(fmt, ...)  aos_log_tag("tag", AOS_LL_INFO, fmt"\n",##__VA_ARGS__)
#define LOG_E(fmt, ...)  aos_log_tag("tag", AOS_LL_ERROR, fmt"\n",##__VA_ARGS__)

#define i2c_write   Sensors_I2C_WriteRegister
#define i2c_read    Sensors_I2C_ReadRegister 
#define delay_ms    aos_msleep
#define get_ms      get_ticks
#define log_i       LOG_I
#define log_e       LOG_E 
#define min(a,b) ((a<b)?a:b)
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    unsigned char data[MAX_PACKET_LENGTH] = {0};
